...
Priority=[]
for i in range (0,No_of_processes):
    print("Enter the Burst time and then Priority for Process " + str(i+1) + ": ")
    Ith_Burst_time=int(input())
    Ith_Priority=int(input())
    Brust_time.insert(i,Ith_Burst_time)
    Priority.insert(i,Ith_Priority)
...
TurnAroundTime=[]
for i in range(0,No_of_processes):
    TurnAroundTime.insert(i,Running_Sum_of_time[i]-Waiting_Time[i])
